...
const httpReducer = (prevHttpState, action) => {
  switch (action.type) {
    case "SEND":
      ...
      return { ...prevHttpState, loading: true };
    case "RESPONSE":
...
const Ingredients = () => {
  ...
  // useCallback to avoid re-rendering everytime and [] to define dependencies
  ...
  const removeIngredientHandler = useCallback((ingredientId) => {
    dispatchHttp({ type: "SEND" });
    fetch(
      `https://react-hooks-update-6bc88.firebaseio.com/ingredients/${ingredientId}.json`,
      {
        method: "DELETE",
      }
    )
  ...
  return (
    <div className="App">
      {httpState.error && (
        <ErrorModal onClose={clearErrorMessage}>{httpState.error}</ErrorModal>
      )}
      <IngredientForm
